...
public class AddControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Product tmp= null;
        try {
            tmp = LoadData.getInstance().getProductByID(nid);
        } catch (Exception e) {
        ...
        if(tmp!=null){
            ...
            Boolean tmp1= null;
            try {
                tmp1 = LoadData.getInstance().insertProduct(nid, nname, nprice, ntype, nsize, nimage1,nimage2,nimage3, ndescription);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            Product product = new Product(nid, nname, Integer.parseInt(nprice), ntype, Integer.parseInt(nsize), nimage1,nimage2,nimage3, ndescription);
            if(tmp1) {
                mess = "Add Success";
                alert = "success";

                Brand brand = new Brand();
                brand.setBrandName(ntype);
                brand.setNewProduct(product);
